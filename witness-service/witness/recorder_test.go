...
func TestRecorder(t *testing.T) {
	...
	rs, err := recorder.RecordsToVote(10)
	require.NoError(err)
	...
	require.NoError(recorder.StartVote(r1))
	rs, err = recorder.RecordsToVote(10)
	require.NoError(err)
	require.Equal(1, len(rs))
	require.NoError(recorder.StartVote(r2))
	require.Error(recorder.StartVote(r2))
	require.NoError(recorder.MarkAsVoted(r1, "tx1"))
	require.Error(recorder.MarkAsVoted(r1, "tx1"))
	require.NoError(recorder.MarkAsVoted(r2, "tx2"))
	rs, err = recorder.RecordsToVote(1)
	require.NoError(err)
