...
func (s *SolRecorder) Transfer(id common.Hash) (*Transfer, error) {
	row := s.store.DB().QueryRow(
		fmt.Sprintf("SELECT `cashier`, `token`, `tidx`, `sender`, `txSender`, `recipient`, `ataOwner`, `amount`, `payload`, `fee`, `id`, `txSignature`, `txTimestamp`, `status`, `relayer`, `lastValidBlockHeight` FROM %s WHERE `id`=?", s.transferTableName),
		id.Hex(),
	)
...
func (s *SolRecorder) assembleTransfer(scan func(dest ...interface{}) error) (*SOLRawTransaction, error) {
	...
	if err := scan(&cashier, &token, &tx.index, &sender, &txSender, &recipient, &ataOwner, &rawAmount, &payload, &fee, &id, &signature, &timestamp, &tx.status, &relayer, &lastValidBlockHeight); err != nil {
...
func (s *SolRecorder) HeightsOfStaleTransfers(cashier util.Address) ([]uint64, error) {
	return []uint64{}, nil
}
