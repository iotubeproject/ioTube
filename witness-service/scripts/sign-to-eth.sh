#!/bin/bash
# Sign a witness signature for Ethereum-compatible chain destinations
# Usage: ./sign-to-eth.sh -config <config.yaml> -secret <secret.yaml> -cashier <id> -cotoken <address> -index <num> -sender <address> -recipient <address> -amount <amount>
...
if [[ $# -eq 0 ]]; then
    echo "Usage: $0 -config <config.yaml> -secret <secret.yaml> -cashier <id> -cotoken <address> -index <num> -sender <address> -recipient <address> -amount <amount>"
    echo ""
    echo "Options:"
    ...
    echo "  -cotoken   Co-token address (destination token)"
