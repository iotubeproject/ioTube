    ...
    function onReceive(address _sender, ICrosschainToken _token, uint256 _amount, bytes calldata _payload) external {
        ...
        if (fee > 0) {
            ...
            try r.getAmountsOut(_amount, path) returns (uint256[] memory _amounts) {
                ...
                require(_token.transfer(_sender, _amount), "SolanaHub: estimate amounts out failed");
                return;
            }
            if (amounts[1] < fee) {
                require(_token.transfer(_sender, _amount), "SolanaHub: transfer failed");
                return;
            }
            require(_token.approve(router, _amount), "SolanaHub: approve router failed");
        ...
        require(_token.approve(address(c), _amount), "SolanaHub: approve cashier failed");
