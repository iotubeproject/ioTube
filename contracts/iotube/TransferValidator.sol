...
contract TransferValidator is Pausable {
    ...
    function submitMulti(
        bytes[] memory cashiers,
        address[] memory tokenAddrs,
        uint256[] memory indexes,
        bytes[] memory senders,
        address[] memory recipients,
        uint256[] memory amounts,
        bytes memory signatures
    ) public whenNotPaused {
        require(
            cashiers.length == tokenAddrs.length && tokenAddrs.length == indexes.length
                && indexes.length == senders.length && senders.length == recipients.length
        ...
        for (uint256 i = 0; i < cashiers.length; i++) {
            ...
            require(
                minters[getTokenGroup(tokenAddrs[i])].mint(tokenAddrs[i], recipients[i], amounts[i]),
