const TokenCashier = artifacts.require('TokenCashierWithPayload');
...
contract('TokenCashierWithPayload', function([owner, minter, sender, receiver, fakeTokenAddress1, fakeTokenAddress2]) {
    beforeEach(async function() {
        ...
        await this.cashier.addTokenList(
            this.mintableTokenList.address,
            '0x0000000000000000000000000000000000000000',
        );
        await this.cashier.addTokenList(
            this.standardTokenList.address,
            this.tokenSafe.address,
        );
    });
    ...
    describe("deposit", function() {
        ...
        describe("burn", function() {
            ...
            describe("enough quota", function() {
                ...
                it('insufficient deposit fee', async function() {
                    ...
                    await assertAsyncThrows(this.cashier.methods['deposit(address,uint256)'](this.shadowToken.address, 1000, {from: sender, value: 12}));
                    await assertAsyncThrows(this.cashier.methods['deposit(address,uint256,bytes)'](this.shadowToken.address, 1000, "", {from: sender, value: 12}));
                });
                describe('success', function() {
                    ...
                    it('no fee with payload', async function() {
                        const payload = "0x0123456789abcdef"
                        const depositResponse = await this.cashier.methods['deposit(address,uint256,bytes)'](this.shadowToken.address, 1000, payload, {from: sender});
                        assert.equal(await this.shadowToken.balanceOf(sender), 9999999000);
                        assert.equal(await this.cashier.count(this.shadowToken.address), 1);
                        assert.equal(depositResponse.logs.length, 1);
                        assert.equal(depositResponse.logs[0].event, "Receipt");
                        ...
                        assert.equal(depositResponse.logs[0].args.amount, 1000);
                        assert.equal(depositResponse.logs[0].args.fee, 0);
                        assert.equal(depositResponse.logs[0].args.payload, payload);
                    });
                    it('depositTo with fee', async function() {
                        await this.cashier.setDepositFee(1234);
                        const depositResponse = await this.cashier.methods['depositTo(address,address,uint256)'](this.shadowToken.address, receiver, 500, {from: sender, value: 1234});
                        ...
                        assert.equal(depositResponse.logs[0].args.payload, null);
                    ...
                    it('depositTo with payload and fee', async function() {
                        await this.cashier.setDepositFee(1234);
                        const payload = "0x0123456789abcdef"
                        const depositResponse = await this.cashier.methods['depositTo(address,address,uint256,bytes)'](this.shadowToken.address, receiver, 500, payload, {from: sender, value: 1234});
                        assert.equal(await this.shadowToken.balanceOf(sender), 9999999500);
                        assert.equal(await this.cashier.count(this.shadowToken.address), 1);
                        assert.equal(await web3.eth.getBalance(this.cashier.address), 1234);
                        assert.equal(depositResponse.logs.length, 1);
        ...
        describe("deposit into safe", function() {
            beforeEach(async function() {
            ...
            it("no payload", async function() {
                ...
                depositResponse = await this.cashier.methods['depositTo(address,address,uint256)'](this.shadowToken.address, receiver, 500, {from: sender, value: 1234});
                ...
                depositResponse = await this.cashier.methods['depositTo(address,address,uint256)']("0x0000000000000000000000000000000000000000", receiver, 50000, {from: sender, value: 54321});
            ...
            it("with payload", async function() {
                const payload = "0x0123456789abcdef"
                let depositResponse = await this.cashier.methods['deposit(address,uint256,bytes)'](this.shadowToken.address, 1000, payload, {from: sender});
                assert.equal(await this.shadowToken.balanceOf(sender), 9999999000);
                assert.equal(await this.shadowToken.balanceOf(this.tokenSafe.address), 1000);
                assert.equal(await this.cashier.count(this.shadowToken.address), 1);
                assert.equal(await web3.eth.getBalance(this.cashier.address), 0);
                ...
                assert.equal(depositResponse.logs[0].args.payload, payload);
                await this.cashier.setDepositFee(123);
                depositResponse = await this.cashier.methods['depositTo(address,address,uint256,bytes)'](this.shadowToken.address, receiver, 500, payload, {from: sender, value: 1234});
                assert.equal(await this.shadowToken.balanceOf(sender), 9999998500);
                assert.equal(await this.shadowToken.balanceOf(this.tokenSafe.address), 1500);
                assert.equal(await this.cashier.count(this.shadowToken.address), 2);
                assert.equal(await web3.eth.getBalance(this.cashier.address), 1234);
                ...
                const balanceBeforeDeposit = await web3.eth.getBalance(sender);
                // const gasPrice = await web3.eth.getGasPrice();
                depositResponse = await this.cashier.methods['depositTo(address,address,uint256,bytes)']("0x0000000000000000000000000000000000000000", receiver, 50000, payload, {from: sender, value: 54321});
                assert.equal(await web3.eth.getBalance(sender), web3.utils.toBN(balanceBeforeDeposit).sub(web3.utils.toBN(depositResponse.receipt.gasUsed * depositResponse.receipt.effectiveGasPrice + 54321)));
                assert.equal(await this.weth.balanceOf(this.tokenSafe.address), 50000);
                assert.equal(await this.cashier.count(this.weth.address), 1);
                assert.equal(await web3.eth.getBalance(this.cashier.address), 5555);
